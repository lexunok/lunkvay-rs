use crate::{
    api::friends::{create_friendship_label, delete_friendship_label, update_friendship_status, CreateFriendshipLabelRequest, UpdateFriendshipStatusRequest},
...
pub fn FullFriendCard(friend: Friendship, refetch_friends: Callback<()>) -> impl IntoView {
    ...
    let create_label_action = Action::new_local(
        |(friendship_id, label): &(Uuid, String)|  {
    ...
    let delete_label_action = Action::new_local(|label_id: &Uuid| {
        ...
        async move {
            delete_friendship_label(label_id).await
        }
    });
